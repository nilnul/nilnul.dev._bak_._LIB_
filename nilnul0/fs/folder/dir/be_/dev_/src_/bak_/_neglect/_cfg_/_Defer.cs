...
namespace nilnul.fs.folder.be_.deV_.src_.bak_._neglect._cfg_
{
	...
	public static class _DeferX
	{
		...
		{
			var t = _defer._OfDotNilnulAsXmlX._Be_blobOfNulable(blob0nul);
			if (t is null)
			{
		...
		{
			var t = _defer._OfDotNilnulAsXmlX.Be(s);
			if (t is null)
			{
				return nilnul.fs.folder.be_.deV_.src_.bak_.aft.Anto.Be(s, git);
			}
			return t;
