...
namespace nilnul.fs.address_.shields.dev_.bak
{
	public  class Save
	{
		...
		public Save(nilnul.fs.address_.shields.dev_._bak.Io cfg)
		{
			_cfg = cfg;
		}
		...
		public Save(CancellationToken cancel, nilnul.win.prog_.Git git=null)
			: this(
	nilnul.fs.address_.shields.dev_._bak.io_._CfgX.Create(cancel, git)
)
		{
		...
		{
			var bak = new nilnul.fs.address_.shields.dev_.Bak(cfg);
			...
			try
			{
				...
				System.Threading.Tasks.Task.Factory.StartNew(
					() => {
						bak.Exe(_shieldsInAddresses_assumeDisjointAddresses);
					}
					,
					cfg.cancel
					,
