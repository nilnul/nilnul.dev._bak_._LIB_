...
namespace nilnul.fs.git.module.repo._cfg_.remotes_.bak_.awGlVs.ensure
{
	static public class _UpsertOmegaX
	{
			...
			)
		{
			...
			{
				var urls = nilnul.fs.git.module.repo._cfg_.remote_.bak_.awGlVs_.last_.next._CreateX.Urls(gitTop, git).Select(
					u=> nilnul.web.url.op_.unary_._DelKeyX.Op(u).ToString()
				);
				return $@"created awGlVs_last_next of urls{nilnul.txt.accumulate_.join_.DuoLineBreak.Singleton.accumulate(urls)}";
			}
			else
			{
				return remote_.bak_.awGlVs_.aliases.vers.last.nulable.urls._EnsureX.Msg(
					gitTop,
					git
				);
			}
