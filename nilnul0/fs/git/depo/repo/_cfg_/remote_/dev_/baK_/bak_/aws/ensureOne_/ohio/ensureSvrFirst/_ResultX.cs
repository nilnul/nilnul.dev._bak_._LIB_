...
namespace nilnul.fs.git.module.repo._cfg_.remote_.bak_.aws.ensureOne_.ohio.ensureSvrFirst
{
	static public class _ResultX
	{
		...
		)
		{
			try
			{
				return new nilnul._op.result_.Explain<bool>(_EnsureSvrFirstX.NewlyCreated(gitTop, svrRepoName, git)
				);
			}
			catch (Exception e)
			{
