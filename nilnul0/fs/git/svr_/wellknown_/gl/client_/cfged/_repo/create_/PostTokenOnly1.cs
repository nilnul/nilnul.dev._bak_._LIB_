...
{
	static public class _PostWithTokenOnlyX1
	{
		...
		{
			 return  nilnul.fs.git.svr_.gitlab.acc_.token._repo._CreateX.Result(
				 new web._url._query._entry.Term(Cfged.Ensure().Password)
				 , 
				 prj
			);
		}
		...
		{
			 return  nilnul.fs.git.svr_.gitlab.acc_.token._repo._CreateX.Result1(
				 new web._url._query._entry.Term(Cfged.Ensure().Password)
				 , 
				 prj
			);
