		...
		public void _startChildTask_0partic(string folder)
		{
			...
			var waited = cfg.semaphore.Wait(
				10 * 60 * 1000/* when this time duration is set too short, the popped up windows would be too many;and when we handle them manually, new window would be popped out, increasing the total number of windows; So when we manually handle a window, the new window shall not be popped out before we finish one; thus, supposing we need 10minuts to handle one window, the time here shall be set 10m
				               * we generally run the program at night and artificially monitor it the next day; soe the total number of popup windows shall be not too much; We shall control the number of popups per hour in a trivia number, eg: 10minutes would make 6per hour; 
				               And the next day we can cancel the running process  (or set a flag or new timespan for nonSemaphore process ), and the popups are grouped in os taskbar; if the number is too much, they are forming a menu; if the number is not too much, they are forming a row of miniature windows, and this would make it easy to be monitored whether it's waiting4input;
				               */
			...
			if (!waited)
			{
				Task.Factory.StartNew(
					...
					(t) =>
					{
						Trace.TraceInformation($"{(this.GetType().FullName)}.{nameof(_startChildTask_0partic)}({folder})  --- task continuing outside semaphore");
						...
						{
							Trace.TraceError($"when configuring baked.time for {folder} there is exception:{e}; (see rethrown exception); this may indicate that the folder is not a git module, and no '.git' subfolder is found inside.");
							//throw;
						}
