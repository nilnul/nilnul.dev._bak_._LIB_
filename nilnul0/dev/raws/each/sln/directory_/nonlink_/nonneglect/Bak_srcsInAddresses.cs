		...
		)
		{
			...
			)
			{
				case nilnul.fs.folder.categorize_.git_._plainWorkRepo.Ret.Plain:
					...
					new sln_.nonlink.directory_.nonlink_.nonneglect_.giT_.plain.Bak_srcsInAddresses(cfg) { }._exe_ofAddress(shieldTxt);
					break;

				case nilnul.fs.folder.categorize_.git_._plainWorkRepo.Ret.Work:
					new sln.directory_.nonlink_.nonneglect_.giT_.work.Bak_srcsInAddresses(cfg, innerModules) {module=this.module }._Exe(
