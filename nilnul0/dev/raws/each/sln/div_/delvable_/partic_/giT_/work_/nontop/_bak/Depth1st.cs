			...
			try
			{
				...
				try
				{
					...
					)
					{
						...
						var child = dir.FullName;// System.IO.Path.Combine(_location__nonrootWork, dnt);
						...
						switch (
							...
							default:
								...
								Trace.TraceError(
									nilnul._xml.node_.txt._CodecX.Escape(

									$@"{child} as a dir under a git work nontop is triaged into an unknown category other than Work|Repo;  if the dir is a crashed/corrupted module,or the owner is not the current user, then git would indicate it's plain of the trichonotomy; we would not traverse into it, and plz check/rectify it.
						It can also be a symlink with the target missing;
"
									)
								);
								break;
