...
namespace nilnul.dev.srcs.bak_._retVoid.sln_.nonlink.div_.nonlink_.git_.work_.top_.undone_.moduleUndone_
{
	/// <summary>
	/// </summary>
				...
				foreach (var child in nilnul.fs.folder.dirs_.excludeVs_._NormalX.EnumerateAsAddresses(folder))
				{
					...
					if (nilnul.fs.git.module.dir.be_._WorkX.Be(child, git))
					{
						...
						)
						{
							...
							//////div_.normal_.git_._TopX._Exe(
							//////	child,
							//////	cancel,
							//////	git
						 //////  );

							sln_.nonlink.div_.nonlink_.git_._TopX._Exe(child,cancel, git);

						}
						else
						{
							//////sln.div_.git_.work_._NonTopX._Exe(child, /*log,*/ cancel, git);
							sln_.nonlink.directory_.nonlink_.git_.work_._NontopX._Exe( child, cancel,git);
