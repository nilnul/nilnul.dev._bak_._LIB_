...
namespace nilnul.dev.srcs.bak_._retVoid.sln_.nonlink_.git_.work_.top.div_.nonlink_.git_.work_.top_.incomplete_
{
	/// <summary>
	/// </summary>
		...
		)
		{
			...
			foreach (var child in nilnul.fs.folder.dirs_.excludeVs_._NormalX.EnumerateAsAddresses(_slnDiv_git_top))
			{
				...
				if (nilnul.fs.git.module.dir.be_._WorkX.Be(child, git))
				{
					...
					{
						directory_.nonlink_.work_._NontopX._Exe(child, /*log,*/ cancel, git);
					}
				}
