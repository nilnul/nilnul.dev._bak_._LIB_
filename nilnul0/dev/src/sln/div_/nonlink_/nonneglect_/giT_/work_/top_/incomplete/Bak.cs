...
namespace nilnul.dev.sln_.nonlink.div_.nonlink_.nonneglect_.git_.work_.top_.incomplete
{
	/// <summary>
	/// </summary>
	public class Bak
...
If a parent cancels itself before its child task is started, the child never starts. If a parent cancels itself after its child task has already started, the child runs to completion unless it has its own cancellation logic. For more information, see Task Cancellation.*/
			...
			try
			{
				new _incomplete_.content.Bak(cfg, innerModules, innerModules2reinclude) {/* module=nilnul.fs.address_.shield_._AddressX1.Create(folder) */}._Exe(folder);
			...
			try
			{
				new _incomplete_.container.Bak(cfg)._Exe(folder, innerModules, innerModules2reinclude);
