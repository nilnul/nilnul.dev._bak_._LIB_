...
namespace nilnul.dev.src.sln.div_.unjoint_.nonneglect_.giT_.work_.nonintent
		...
		)
		{
			try
			{
				if (nilnul.fs.folder_.git_.work.be_._TopX.__Be_ofAddress(folderAddress))
				{
					...
					new work_.top.Bak_srcsInAddresses(cfg)._void(folderAddress);
				}
				else
				{

					new work_.nontop._bak.Depth1st(cfg, innerModules,innerModules2reinclude,module)._exe(folderAddress);
				}
			}
