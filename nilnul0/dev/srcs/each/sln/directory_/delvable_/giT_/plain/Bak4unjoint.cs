...
namespace nilnul.dev.srcs.each.sln.directory_.delvable_.giT_.plain
{
	//[Obsolete()]
	public class Bak4unjoint
	{
		...
		public void __exe(string work)
		{
			...
			{
				Trace.TraceWarning(
					$"{work} is not partaking;"
				);

				new delvable_.unpartic_.giT_.plain.Bak_srcsInAddresses(
			...
			new delvable_.partic_.giT_.plain.Bak_srcsInAddresses(cfg, innerModules, innerModules2reinclude) { module = this.module }._exe(work);
