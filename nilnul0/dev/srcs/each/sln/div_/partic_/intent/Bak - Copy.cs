...
namespace nilnul.dev.srcs.each.sln.div_.partic_.intent
{
	...
	public  class Bak_srcsInAddress
	{
		...
		)
		{
			...
			{
				new sln.div_.link_.partic_.intent.Bak_srcsInAddress(_cfg,innerModules,innerModules2reinclude,module).exe();

			}
			else
			{
				new sln.div_.delvable_.partic_.intent.Bak_srcsInAddress(_cfg,innerModules,innerModules2reinclude,module).vod();
			}
