	...
	public static class _BakX
	{
			...
			nilnul.win.prog_.Git git = null)
		{
			...
			try
			{
				...
				var remotes = nilnul.fs.git.module.repo._cfg_.remotes_.dev_.baK._VwX._Name0nules_0depo(
					_location
					,
					clients
				).ToArray();
				...
				Trace.TraceInformation($"pushing {bakTag} for { _location}");
				...
				try
				{
					...
					foreach (var remote in latestRemotes)
					{
						...
						results.Add(
								...
								nilnul.os.prog_.git.run_.lurk_.exit._CodX._Cod_ofAddress_ofArgument_lurkAssumeNum(
									_location,
									$"push {remote} tag {bakTag}"
