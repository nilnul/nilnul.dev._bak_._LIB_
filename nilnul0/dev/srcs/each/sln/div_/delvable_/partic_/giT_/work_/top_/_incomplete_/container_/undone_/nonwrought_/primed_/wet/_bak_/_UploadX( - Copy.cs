...
namespace nilnul.dev.srcs.each.sln.div_.delvable_.partic_.giT_.work_.top_._incomplete_.container_.undone_.nonwrought_.primed_.stageDirty._bak_
{
	/// <summary>
	...
	public static class _UploadX
	{
		...
		)
		{
			...
			try
			{
				bakBranch = _bak_._upload_.branch_._shelf.put._BranchOfNewContentX._Branch(
				   _location
				   ,
				   /*innerModules,*/ cancel
				   ,
				...
				var ver = nilnul.txt_.vered._VerX.Txt(
					bakBranch
				);
				...
				{
					Trace.TraceError($"{_location} is untouched since {time}");
				}
			}
			catch (
				OperationCanceledException ex		//cancelled
			)
			{
				throw;
