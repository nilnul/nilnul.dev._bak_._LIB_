			...
			try
			{
				switch (nilnul.fs.folder.categorize_.git_.plainWorkRepo_.Deadlined.Category_ofAddress(folderAddress))
				{
					case fs.folder.categorize_.git_._plainWorkRepo.Ret.Plain:
						new partic_.giT_.plain.BakByIntent(cfg, innerModules, innerModules2reinclude, module)._Exe(folderAddress);

						break;
					case fs.folder.categorize_.git_._plainWorkRepo.Ret.Work:
						new partic_. giT_.work.Bak_srcsInAddresses(cfg, innerModules, innerModules2reinclude, module).Exe(folderAddress);
