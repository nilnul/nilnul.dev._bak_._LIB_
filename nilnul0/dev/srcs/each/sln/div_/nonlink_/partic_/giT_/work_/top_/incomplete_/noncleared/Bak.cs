...
using nilnul.dev.sln_.nonlink.div_.nonlink_.nonneglect_.git_.work_.top_.incomplete;
using System;

namespace nilnul.dev.sln.div_.module_.nonlink_.nonneglect_.incomplete_.noncleared
{
	/// <summary>
	/// </summary>
	/// 
...
If a parent cancels itself before its child task is started, the child never starts. If a parent cancels itself after its child task has already started, the child runs to completion unless it has its own cancellation logic. For more information, see Task Cancellation.*/
			...
			new nilnul.dev.sln_.nonlink.div_.nonlink_.nonneglect_.git_.work_.top_._incomplete_.content.Bak(cfg, innerModules,innerModules2reinclude) {/* module=nilnul.fs.address_.shield_._AddressX1.Create(folder) */}._Exe(folder);

			cfg.cancel.ThrowIfCancellationRequested();

			new _noncleared_.container.Bak(cfg)._Exe(folder, innerModules,innerModules2reinclude);
