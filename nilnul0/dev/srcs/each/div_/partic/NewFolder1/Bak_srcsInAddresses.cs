...
//		}
		...
		)
		{
			...
			{
				case 0: //src
					new nilnul.dev.srcs.each.div_.delvable_.dispat_.partic.Bak(cfg).exe(
					  // _div_inSrc.ToString()
					);

					break;
				case 1: //sln
					new nilnul.dev.srcs.each.sln_.delvable_.partic.Bak_srcsInAddresses(cfg)._vod_ofAddress(_div_inSrc.ToString());
					break;

				default:    //inside sln
					new nilnul.dev.srcs.each.sln.directory_.delvable_.partic.Bak_srcsInAddresses(cfg,new ConcurrentBag<fs._address.DivI>())._exe(
						 _div_inSrc
						);
					break;
			}
